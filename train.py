...
wandb.init(project="textdiff-training")  # Initialize WandB
...
def main(args, extractor, data_loader):
    ...
    wandb.log({"trainable_params": num_trainable_params})  # Log trainable parameters
    ...
    for epoch in range(args['max_training']):
        ...
        wandb.log({"epoch": epoch, "loss": loss.item(), "epoch_time": epoch_time})
        ...
        with torch.no_grad():
            ...
            wandb.log({"eval_dice": eval_dice, "eval_iou": eval_iou, "inference_time": eval_time})
    ...
    wandb.log({"total_train_time": train_time})
    wandb.finish()
