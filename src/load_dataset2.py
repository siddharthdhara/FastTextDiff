...
HUGGINGFACE_TOKEN = "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"
...
def enable_flash_attention(model):
    """Modify ModernBERT to use Flash Attention during inference."""
    model.to("cuda")  # Move model to GPU before modifying layers
    
    for layer in model.layers:  # Ensure correct layer access
        if hasattr(layer, "self_attn"):  # Check if self-attention exists
...
class Mixdataset(Dataset):
    def __init__(self, dataset_path: str, row_text: str, joint_transform: Callable = None,
        ...
        # self.bert_embedding = AutoModel.from_pretrained('google-bert/bert-base-cased')
        # self.tokenizer =  AutoTokenizer.from_pretrained('emilyalsentzer/Bio_ClinicalBERT')
        # self.tokenizer =  AutoTokenizer.from_pretrained('answerdotai/ModernBERT-base')
        # self.bert_embedding = AutoModel.from_pretrained('emilyalsentzer/Bio_ClinicalBERT')
        # self.bert_embedding = AutoModel.from_pretrained('answerdotai/ModernBERT-base')
        # self.tokenizer =  AutoTokenizer.from_pretrained('siddharthdhara17/ClinicalModernBERT-3epochs', use_auth_token=True)
        # self.bert_embedding = AutoModel.from_pretrained('siddharthdhara17/ClinicalModernBERT-3epochs', use_auth_token=True)
        # self.tokenizer =  AutoTokenizer.from_pretrained('siddharthdhara17/ClinicalModernBERT-4epochs', use_auth_token=True)
        # self.bert_embedding = AutoModel.from_pretrained('siddharthdhara17/ClinicalModernBERT-4epochs', use_auth_token=True)
        self.tokenizer =  AutoTokenizer.from_pretrained('siddharthdhara17/ClinicalModernBERT-4epochs', use_auth_token=True)
        self.bert_embedding = AutoModel.from_pretrained('siddharthdhara17/ClinicalModernBERT-4epochs', use_auth_token=True)
        # self.tokenizer =  AutoTokenizer.from_pretrained('siddharthdhara17/ClinicalModernBERT-KD-10epochs', use_auth_token=True)
        # self.bert_embedding = AutoModel.from_pretrained('siddharthdhara17/ClinicalModernBERT-KD-10epochs', use_auth_token=True)
        enable_flash_attention(self.bert_embedding)  # Apply Flash Attention 
    ...
    def __getitem__(self, idx):
        ...
        if 'MosMedDataPlus' in self.dataset_path:
            mask_name = img_name
        elif 'monu' in self.dataset_path:
            mask_name = img_name[:-3]+'png'
        else:
            mask_name = img_name if img_name.startswith("mask_") else "mask_" + img_name
        ...
        with torch.no_grad():
            text_inputs = self.tokenizer(text,return_tensors='pt', padding='max_length', truncation=True, max_length=10).to("cuda")
            text_token = self.bert_embedding(**text_inputs)
        text = text_token['last_hidden_state'].to("cuda").cpu().numpy().squeeze()
